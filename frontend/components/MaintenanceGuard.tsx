...
import api from "@/lib/api"; // <-- ضفنا الـ api عشان نكلم الباك إند
...
export default function MaintenanceGuard({
  ...
  useEffect(() => {
    setIsMounted(true);
    // هنجيب حالة الصيانة من السيرفر أول ما الموقع يفتح
  ...
  if (!isMounted || loading) return null; // استنى ثانية لحد ما يتأكد من حالة السيرفر
  ...
  // لو وضع الصيانة شغال واليوزر مش أدمن، هنظهرله التصميم بتاع المهندس اللي نايم
  if (maintenanceMode && !isAdmin) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans" dir="rtl">
        {/* ... (نفس تصميم الأنيميشن بتاعك بالظبط زي ما هو، سيبه زي ما كان) ... */}
        ...
        >
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
            <div className="inline-block px-4 py-1.5 rounded-full bg-white/10 border border-white/5 text-sm font-semibold text-white/80 mb-6">🛠️ تحديثات النظام</div>
            <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight leading-tight">في صيانة دلوقتي</h1>
            <p className="text-2xl text-emerald-400 font-bold mb-8">يلا عشان هنرش مياه 🧹💦</p>
            <div className="h-px w-full bg-gradient-to-r from-transparent via-white/15 to-transparent mb-8" />
            <p className="text-zinc-400 text-base md:text-lg leading-relaxed mb-8"><span className="text-white font-semibold"></span>... <br/>    انا مريح شويه متوجعوش دماغي</p>
